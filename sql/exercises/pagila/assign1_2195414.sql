...
where i.inventory_id is null and rental_id is null;
...
--    , `minutes` and `seconds`.
select round(avg(length / 60), 0) as hours,
       round(avg(length % 60), 0) as minutes,
       round(avg(length * 0), 0)  as seconds
from film;
...
select distinct i.film_id, f.title, f.release_year
from inventory i
         inner join film f on i.film_id = f.film_id
where i.store_id = 1
  and i.film_id in
